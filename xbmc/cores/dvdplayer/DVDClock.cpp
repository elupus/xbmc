...
int CDVDClock::UpdateFramerate(double fps, double* interval /*= NULL*/)
{
  ...
  double adjust = CSettings::Get().GetNumber("videoplayer.maxspeedadjust");
  //set the speed of the videoreferenceclock based on fps, refreshrate and maximum speed adjust set by user
  if (adjust > 0.05)
  {
    if (weight / MathUtils::round_int(weight) < 1.0 + adjust / 100.0
    &&  weight / MathUtils::round_int(weight) > 1.0 - adjust / 100.0)
